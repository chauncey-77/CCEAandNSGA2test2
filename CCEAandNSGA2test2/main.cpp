...
int F_INDIVIDUAL_cmp(const F_INDIVIDUAL &ind1, const F_INDIVIDUAL &ind2);
...
int F_INDIVIDUAL_cmp(const F_INDIVIDUAL &ind1, const F_INDIVIDUAL &ind2)  //1为支配，2为反支配，3为非支配
{
	if ((ind1.m_f1_obj >= ind2.m_f1_obj && ind1.m_f2_obj > ind2.m_f2_obj) ||
		(ind1.m_f1_obj > ind2.m_f1_obj && ind1.m_f2_obj >= ind2.m_f2_obj))
	{
		return 2;
	};
	if ((ind1.m_f1_obj <= ind2.m_f1_obj && ind1.m_f2_obj < ind2.m_f2_obj) ||
...
bool isCanAddToTmpPop(const F_INDIVIDUAL &ind)
{
	...
	{
		if (F_INDIVIDUAL_cmp(ind, TMP_POP[i]) == 1)
		{
			return true;
		}
		if (F_INDIVIDUAL_cmp(TMP_POP[i], ind) == 1)
		{
			return false;
		}
...
void addToTmpPop(const F_INDIVIDUAL &ind)
{
	...
	{
		if ((F_INDIVIDUAL_cmp(TMP_POP[i], ind) == 1) ||
			(F_INDIVIDUAL_cmp(TMP_POP[i], ind) == 3))
		{
...
void F1_POPULATION::select(int &p1, int &p2)
{
	...
	for (int i = 0; i < IN_POP_SIZE; ++i)
	{
		total_fitness += m_population[i].m_fitness;
	}
	double arr[IN_POP_SIZE];
	arr[0] = m_population[0].m_fitness / total_fitness;
	for (int i = 1; i < IN_POP_SIZE; ++i)
	{
		arr[i] = arr[i-1] + m_population[i].m_fitness / total_fitness;
	}
	int res[2];
	for (int k = 0; k < 2;)
	{
		double r = createZeroToOneRand();
		int i = 0;
		while (arr[i] < r) i++;
		res[k] = i;
		if (i >= 0 && i < IN_POP_SIZE) ++k;
	}
	p1 = res[0];
	p2 = res[1];*/
}
...
bool isCanAddToResPop(const F_INDIVIDUAL &ind)
{
	...
	for (int i = 0; i < RES_POP.size(); ++i)
	{
		if (F_INDIVIDUAL_cmp(ind, RES_POP[i]) == 1)
		{
			return true;
		}
		if (F_INDIVIDUAL_cmp(RES_POP[i], ind) == 1)
		{
			return false;
		}
...
void addTmpPopToResPop()
{
	for (int i = 0; i < TMP_POP.size(); ++i)
	{
		if (isCanAddToResPop(TMP_POP[i]) == true)
		{
			...
			{
				if ((F_INDIVIDUAL_cmp(RES_POP[n], TMP_POP[i]) == 1) ||
					(F_INDIVIDUAL_cmp(RES_POP[n], TMP_POP[i]) == 3))
				{
